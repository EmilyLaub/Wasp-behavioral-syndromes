...
mcmc_cor_AE_Poisson <- mcmc_A_E_us$VCV[,"traitseconds_moving:traitchambers_entered.Wasp.ID"]/ (sqrt(mcmc_A_E_us$VCV[,"traitseconds_moving:traitseconds_moving.Wasp.ID"])*
                                                                                                     sqrt(mcmc_A_E_us$VCV[,"traitchambers_entered:traitchambers_entered.Wasp.ID"]))

mean(mcmc_cor_AE_Poisson)

HPDinterval(mcmc_cor_AE_Poisson)
...
mcmc_A_A_us <- MCMCglmm(cbind(scale(seconds_moving), scale(bodily_contact_time)) ~ trait-1 + at.level(trait,2):(as.numeric(Dummy.Color_clean)) + trait:scale(Trial, scale = FALSE) +
                          trait:scale(weight),
                        random =~ us(trait):Wasp.ID,
                        rcov =~ us(trait):units,
...
mcmc_Ac_Agg_us <- MCMCglmm(cbind(scale(seconds_moving), scale(Log_aggression)) ~ trait-1 + trait:scale(Trial, scale = FALSE) +
                             trait:scale(weight) + at.level(trait,2):(as.numeric(Dummy.Color_clean)),
                           random =~ us(trait):Wasp.ID,
                           rcov =~ us(trait):units,
...
str(personality)
min(personality$chambers_entered)
...
mcmc_Agg_Neu_us <- MCMCglmm(cbind(scale(Log_aggression), (anntenation)) ~ trait-1 + trait:scale(Trial, scale = FALSE) +
                              trait:scale(weight) + at.level(trait,2):(as.numeric(Dummy.Color_clean)) + at.level(trait,1):(as.numeric(Dummy.Color_clean)),
                            random =~ us(trait):Wasp.ID,
                            rcov =~ us(trait):units,
...
mcmc_Aff_Ex_us <- MCMCglmm(cbind(scale(bodily_contact_time), (chambers_entered)) ~ trait-1 + trait:scale(Trial, scale = FALSE) +
                             trait:scale(weight) + at.level(trait,1):scale(as.numeric(Dummy.Color_clean)),
                           random =~ us(trait):Wasp.ID,
                           rcov =~ us(trait):units,
...
################# GRAPH THIS THIS AND MAKE SURE TO WRITE THE FILE
